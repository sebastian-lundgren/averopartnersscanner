...
async def upload_images(
    ...
    for uf in files:
        ...
        local_img, tmp_del = materialize_local_path(stored_path, suffix=".upload")
        try:
            try:
        ...
        needs_review = pred_result.confidence < strong or pred_result.status != models.ReviewStatus.SKILT_FUNNET

        evidence_rel = None
        if pred_result.bbox_norm:
            ev_name = f"ev_{img.id}_{uuid.uuid4().hex[:8]}.jpg"
            evidence_rel = save_evidence_crop(stored_path, ev_name, pred_result.bbox_norm)
            if evidence_rel:
        ...
        created.append({"image_id": img.id, "prediction_id": pred.id})
